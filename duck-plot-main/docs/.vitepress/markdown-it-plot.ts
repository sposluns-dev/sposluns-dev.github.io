  ...
  md.use(container, "plot", {
    render(tokens, idx) {
      if (tokens[idx].nesting === 1) {
        ...
        if (token.info === "js-vue")
          content = content.replace(/"{{([^}]*)}}"/g, "$1");
        content = content.replace(/\bMath\.random\b/g, "d3.randomLcg(42)");
        content = content.replace(
          /\bd3\.(random(?!Lcg)\w+)\b/g,
          "d3.$1.source(d3.randomLcg(42))"
        );
        content = content.replace(
          /\bd3\.shuffle\b/g,
          "d3.shuffler(d3.randomLcg(42))"
        );
        ...
        const suffix = `\n${
          directives.includes("hidden")
            ...
            : href
            ? `<a class="plot-fork no-icon" href="${md.utils.escapeHtml(
                href
              )}" target="_blank" title="Open on Observable">Fork</a>`
            : ""
        }`;
